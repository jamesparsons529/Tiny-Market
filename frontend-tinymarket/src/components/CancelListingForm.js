...
const CancelListingForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    doContractCall({
      ...
      onFinish: (data) => {
        ...
        window.alert("Cancel Listing Successful");
        setListingId('');
      },
      onCancel: () => {
        console.log("onCancel:", "Transaction was canceled");
        window.alert("Cancel Listing Failed");
      },
      onError: (error) => {
        console.error("Error during contract call:", error);
        window.alert("An error occurred while cancelling the listing. Please try again.");
      }
    });
  };
  ...
  return (
    <div className="purchase-container">
      <h1 className="purchase-title">Cancel NFT Listing</h1>
      ...
      <form id="fulfillForm" className="purchase-form" onSubmit={handleSubmit}>
        ...
        <button type="submit" className="submit-button">Cancel Listing</button>
      </form>
    </div>
  );
};
