...
;; token definitions
;;
(define-non-fungible-token sleepy-stripes uint) ;; Defines new NFT

;; constants
;;
(define-constant contract-owner 'ST1NWPSRC02Z9A20RHSBGDEDG9H8CHS6ENJ2N3TTH) ;; Contract owner's principal
(define-constant mint-cost u1000000) ;; 0.5 STX in microSTX (1 STX = 1,000,000 microSTX)
...
(define-data-var token-hash (string-ascii 100) "QmWjPvRmBEK4Xw4XNcmWw2w8dRhN784PmbtKTHsiHHHs2V") ;; IPFS hash
...
(define-read-only (get-owner (token-id uint)) 
    (ok (nft-get-owner? sleepy-stripes token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal)) ;; Checks if sender is tx-sender
    (begin 
        ...
        (nft-transfer? sleepy-stripes token-id sender recipient)
    )
) 
...
(define-public (mint (recipient principal))
    (let (
        ...
        (try! (nft-mint? sleepy-stripes token-id recipient)) 
